...
[{"title":"取引先名","date":"YYYY/MM/DD","amount":"¥X,XXX","debit":"借方科目","credit":"貸方科目","tax":"課税仕入(10%)|課税仕入(8%軽減)|非課税|不課税","confidence":"high|mid|low","reason":"根拠50字以内"}]`;
...
// ===== Haikuで摘要・インボイス番号を後処理 =====
async function enrichWithHaiku(apiKey, imageData, items) {
  if (!items || items.length === 0) return items;
  const titles = items.map((it, i) => `[${i}] ${it.title} ¥${it.amount}`).join('\n');
  const prompt = `以下の仕訳リストに対して、画像から摘要とインボイス番号を抽出してください。

仕訳リスト：
${titles}

各仕訳について以下をJSONで返してください（説明文・バッククォート不要）：
[{"idx":0,"memo":"摘要（取引内容・但し書き等）","invoice_number":"T1234567890123または空文字"}]

ルール：
- memoは但し書き・取引内容・品目等から簡潔に（20字以内）
- invoice_numberは「T」+13桁数字。ハイフン入り（T4-1200-...）も有効。なければ空文字
- 全仕訳に対してidxを必ず含めること`;
  ...
  try {
    ...
    parsed.forEach(({ idx, memo, invoice_number }) => {
      if (items[idx]) {
        items[idx].memo = memo || '';
    ...
    console.warn('Haiku後処理エラー:', e.message);
    items.forEach(it => { it.memo = it.memo || ''; it.invoice_number = it.invoice_number || ''; });
  }
  return items;
}
  ...
  if (req.method === 'POST' && req.url === '/api/analyze-chunk') {
    ...
    req.on('end', async () => {
      try {
        ...
        // Haikuで摘要・インボイス番号を後処理
        const firstImageData = imageDataList[0];
        const enrichedItems = await enrichWithHaiku(apiKey, firstImageData, items);

        console.log(`チャンク ${chunkIndex+1}/${totalChunks}: ${rawItems.length}件取得 → ${enrichedItems.length}件（フォーマット:${formatKey}・モード:${line_item_mode}・マスタ${Object.keys(master).length}件適用）`);
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ items: enrichedItems, orientation, line_item_mode }));
      } catch(e) {
