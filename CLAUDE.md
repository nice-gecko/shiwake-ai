...
### CC投げかけ管理（2026-05-17 確立）

Claude Code に投げている依頼文を常に把握する。

- 依頼文を差し替えたら「古い方は破棄、これが最新」と明示する
- 報告が届いたら、どの依頼文の結果かを照合してから内容を扱う
- 古い依頼に対する報告と新しい依頼に対する報告を混同しない
- 並行投入時は各CCに対して「これはどのタスクか」を明示した依頼文を書く
...
- トークン節約の定義: 「合意・指示なく、先回りでコード/SQLを無駄に書くこと」を禁止する意味であり、
  現状把握のための読み込み(コード確認・grep・ファイル閲覧)を削る意味ではない。
- 把握に必要なトークンは惜しまず使う。把握を省いて推測で埋めることを禁止する。
  推測で埋めた誤りは手戻りを生み、節約に反する。
- UI動線・画面上の場所を案内する際は、その要素を開くトリガー(onclick・サイドバーのラベル・
  パネルの表示関数等)を実コードで確認してから述べる。動線を推測で補わない。
- 「〜のはず」「おそらく〜から開く」と書きそうになったら未確認のサイン。確認するか、
  できない場合は「未確認」と明示する。断定しない。
- この原則は指示出し役(Claude)・実装役(Claude Code)の双方に適用する。

### 切り分けの教訓(2026-05-30: v2.8 器のWS漏洩誤診から)

- 画面キャプチャやレスポンスの一部(例: Array(1)の件数)だけを見て、サーバの絞り込み失敗等の
  原因を即断しない。同じ件数でも各WS固有データの可能性がある。
- サーバの挙動(WS絞り込みが効いているか等)は、デタラメ/存在しない入力を1回投げて
  返り(エラー or 全件返却)で切り分けてから結論する。推測で差し戻さない。
- 差し戻しの前に、フロント/サーバ/データのどの層が真因かを実測で1つに確定させる。
  層を取り違えた差し戻しは手戻りを生む。
